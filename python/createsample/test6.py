...
test_beta = cv2.imread(
    ...
    cv2.IMREAD_COLOR)
target = cv2.imread("D:\IdeaProjects\crml\picture\windowtargetwithmask.png", cv2.IMREAD_COLOR)
...
dir = "D:\IdeaProjects\crml\picture\chr_musketeer_out"
...
for file in files:
    ...
    if (max_val_res > global_max):
        global_max = max_val_res
        global_max_path = max_path

print("Globa_max is %f with template %s" % (global_max, global_max_path))
#475.807067871
#Globa_max is 594.433655 with template D:\IdeaProjects\crml\picture\chr_musketeer_out\chr_musketeer_sprite_333.png